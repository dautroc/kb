...
async function readLastLogEntry(logPath) {
    let content;
    try {
...
export function makeStatusCommand() {
    ...
    cmd.description("Show status of the current kb project").action(async () => {
        try {
            ...
            console.log(`Project: ${chalk.bold(project.name)} ${chalk.dim(`(v${project.config.project.version})`)}`);
            console.log(`Root:    ${chalk.dim(project.root)}`);
            console.log(`Wiki:    ${wikiPages} page${wikiPages !== 1 ? "s" : ""}`);
            console.log(`Sources: ${sourceCount} source${sourceCount !== 1 ? "s" : ""}`);
            if (lastEntry !== null) {
