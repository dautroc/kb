...
function printPlan(plan: IngestPlan, sourcePath: string): void {
  ...
  console.log(
    `Source: ${chalk.cyan(sourceBase)} → ${chalk.cyan(`sources/${sourceFilename}`)}`,
  );
  ...
  console.log(
    `  ${chalk.green("+")} ${result.summary.path} ${chalk.gray("(new page)")}`,
  );
  ...
  if (result.updates.length > 0) {
    ...
    for (const u of result.updates) {
      console.log(
        `  ${chalk.yellow("~")} ${u.path} ${chalk.gray(`(${u.reason})`)}`,
      );
    }
  }
...
function printApplied(plan: IngestPlan, sourcePath: string): void {
  const sourceBase = basename(sourcePath);
  const sourceFilename = basename(plan.sourceFile);
  const { result } = plan;

  console.log(
    `${chalk.green("✓")} Ingested ${chalk.cyan(sourceBase)} → ${chalk.cyan(`sources/${sourceFilename}`)}`,
  );
...
export function makeIngestCommand(): Command {
  ...
  cmd
    ...
    .action(
      async (
        sourcePath: string,
        options: { apply: boolean; batch: boolean },
      ) => {
        try {
          ...
          if (options.batch) {
            ...
            if (!s.isDirectory()) {
              console.error(
                chalk.red("Error: --batch requires a directory path"),
              );
              process.exit(1);
            }
