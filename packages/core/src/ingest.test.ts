...
const MOCK_LLM_RESULT: IngestResult = {
  summary: {
...
function createMockLlm(result: IngestResult): LlmAdapter {
  return {
...
function makeTestProject(root: string): Project {
...
async function setupTestProject(root: string): Promise<void> {
  await mkdir(join(root, ".kb"), { recursive: true });
  await mkdir(join(root, "sources"), { recursive: true });
  await mkdir(join(root, "wiki"), { recursive: true });

  await writeFile(
    join(root, ".kb", "schema.md"),
    "# Wiki Schema\n\nStandard KB schema.",
    "utf8",
  );
  await writeFile(
    join(root, "wiki", "_index.md"),
    "# Index\n\n- [[overview]]",
    "utf8",
  );
  await writeFile(
...
describe("ingestSource", () => {
  ...
  it("apply mode writes all files", async () => {
    ...
    await writeFile(
      sourceFile,
      "This is content to apply to the wiki.",
      "utf8",
    );
  ...
  it("batch mode processes multiple sources from a directory", async () => {
    ...
    const plan1 = await ingestSource(
      project,
      join(sourcesDir, "file1.txt"),
      llm,
      { apply: false },
    );
    const plan2 = await ingestSource(
      project,
      join(sourcesDir, "file2.txt"),
      llm,
      { apply: false },
    );
