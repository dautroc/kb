...
describe("initProject", () => {
    ...
    it("throws if .kb/ already exists", async () => {
        await initProject({ name: "my-project", directory: tmpDir });
        await expect(initProject({ name: "my-project", directory: tmpDir })).rejects.toThrow("already initialized");
    });
    it("cleans up .kb/ on initialization failure", async () => {
        ...
        await expect(initProject({ name: "test", directory: tmpDir })).rejects.toThrow();
