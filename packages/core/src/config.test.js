...
`;
    ...
    it("throws when file not found", async () => {
        ...
        await expect(parseConfig(missingPath)).rejects.toThrow(/not found|no such file/i);
    });
...
`;
        const configPath = await writeConfig(toml);
        await expect(parseConfig(configPath)).rejects.toThrow(/directories\.sources/i);
    });
...
`;
        const configPath = await writeConfig(toml);
        await expect(parseConfig(configPath)).rejects.toThrow(/directories\.sources must be a safe relative path/i);
    });
...
`;
        const configPath = await writeConfig(toml);
        await expect(parseConfig(configPath)).rejects.toThrow(/directories\.wiki must be a safe relative path/i);
    });
